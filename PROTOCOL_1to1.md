...
- Include figures inside clean environments such as `\begin{center} ... \includegraphics[width=...]{...} \end{center}` or side-by-side layouts with `\begin{minipage}{0.6\textwidth} ... \end{minipage}% \begin{minipage}{0.4\textwidth} ... \end{minipage}`.

### Rule 4: Sober Academic Styling (Zero Colored Boxes or `tcolorbox` Abuse)
...
- Avoid fragile macros or unstable table syntax (such as `\makecell` inside `tabularx`): use clean, native `tabular` or mathematical `array` structures.
